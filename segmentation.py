...
def grabcut_components(image, mask, num_components=1):
	...
	mask_holes_removed = largest_components(mask, num_components=1, output_bounding_box=False)
...
def segment_wing(mask, wing_left=0.4, wing_right=0.6):
...
def segment_image_file(file_in, folder_out, mode='wings'):
	...
	segmented_image = segment_butterfly(image, approximate=False)[1]
	if mode == 'wings':
		segmented_image = segment_wing(segmented_image) / 255

	print np.min(segmented_image), np.max(segmented_image)
	file_out = os.path.join(folder_out, os.path.basename(file_in))
	cv2.imwrite(file_out, 255*segmented_image)
